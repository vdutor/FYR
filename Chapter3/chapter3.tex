...
So far, the domain of interest has been the hypersphere $\dsphere$ . However, most regression problems in machine learning have inputs defined on the complete Euclidean space. We can address this problem as follows. Consider $x \in \Reals^{d-1}$ to be the input of the dataset and $y \in \Reals$ the corresponding outputs.
\begin{enumerate}
    \item We start by concatenating the datapoint with a bias term $b \in \Reals$ to obtain $x = [x, b] \in \Reals^d$. This operation corresponds to embedding the $(d-1)$ dimensional datapoint on a plane in $\Reals^d$.
    ...
    \item Finally, the function on the sphere can be extended to the whole domain by a homogeneous extension: $g: \Reals^d \rightarrow \Reals$ with $g(x) = \norm{x} f({x \over \norm{x}})$. In other words, by linearily extrapolating the values on the hypersphere we obtain the values of the function in the data plane.
\end{enumerate}
...
\end{equation}
where $t = \frac{\vx^\top \vx'}{\norm{\vx}\norm{\vx'}}$. Indeed, we observe how the first order Arc Cosine kernel factorises in a radial and angular component. This factorisation exactly matches our setup as we can generalise the zonal kernels and its corresponding RKHS of the previous section to the complete domain $\Reals^d$ using the following definition
\begin{equation}
