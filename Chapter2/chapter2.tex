...
Interdomain Gaussian processes use alternative forms of inducing variables such that the resulting sparse GP models consists of a different set of features. \cref{chapter:vish} and \cref{chapter:dnn-as-point-estimate-for-dgps} heavily rely on this paradigm. In interdomain GPs, the inducing variables are obtained using a linear transformation of the GP, which we denote by $u_m = \mathcal{L}_m f$. This redefinition of $\vu$ implies that the expressions of $\Kuu$ and $\kux$ change, but the inference scheme of interdomain GPs and the mathematical expressions for the posterior mean and variance are exactly the same as classic sparse GPs. This is thanks to the linearity of the operator and the preservation of (joint) Gaussian behaviour under linear transformations. A common linear operator is the integral operator with an inducing function $g_m$ \citep{lazaro2009inter}:
\begin{equation*}
...
In what follows, we are going to focus on the theoretical properties of Mercer kernels with the application of Gaussian processes in mind. Some of the theorems and definitions in this section are quoted from the excellent course ``Machine learning with kernel methods''\footnote{\url{https://members.cbio.mines-paristech.fr/~jvert/svn/kernelcourse/course/2021mva}} by Julien Mairal and Jean-Philippe Vert, which I voluntarily auditted during my first year.
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \begin{equation}
      \forall x \in \mathcal{X},\quad k_x: \mathcal{X} \rightarrow \Reals, k_x(x') \mapsto k(x, x')
    \end{equation}
...
\subsection{Connection between GPs and RKHS}

It can be shown that the reproducing kernel belonging to an RKHS is unique and, conversely, that a function can only be the reproducing kernel of a single RKHS. Furthermore, a function $k:\mathcal{X} \times \mathcal{X} \rightarrow \Reals$ is a positive definite kernel if and only if it is a reproducing kernel.


\section{Spectral Representation of Gaussian Processes}

Spectral decomposition of the kernel operator

Kernel Operator $\mathcal{K}$ 

\begin{equation}
  \mathcal{K} f = \int k(x, x') f(x') \calcd{\mu(x')},
\end{equation}
where $\mu$ denotes a measure. In our case this measure will typically stem from the input distribution on $x$ and reduce to $p(x)\calcd{x}$.

It can be shown that $\mathcal{K}$ is positive (i.e. $<f, L f> \ge 0$) and self-adjoint (i.e. $<L f, g> = <f, L g>$).
...
\begin{theorem}[Karhunen-Lo\`eve]
  \begin{equation}
    f = \sum_n \sqrt{\lambda_n} \varepsilon_n \phi_n\quad\text{with}\quad\varepsilon_n \sim \NormDist{0, 1}
  \end{equation}
...
\begin{enumerate}
  ...
  \item Bochner's theorem for stationary kernels
\end{enumerate}
